...
function downloadCat(e) {
  ...
  Promise.all([getCssCatColors, getCssFill])
    .then((data) =>{
      ...
      if (el.classList.contains('savecat-svg')) {
      ...
      }else if (el.classList.contains('savecat-png')){
        //create a canvas
...
savecatPNG.addEventListener('click', downloadCat);
//set a click listener to button to save cat as SVG
savecatSVG.addEventListener('click', downloadCat);
